...
export default function DynamicTable<T extends Record<string, string | number>>({ columns, value, onChange, title, description, addButtonText }: { columns: DynamicTableColumn[]; value: T[]; onChange: (rows: T[]) => void; title: string; description?: string; addButtonText?: string }) {
  const addRow = () => {
    const row = Object.fromEntries(columns.map((c) => [c.key, ''])) as T;
    onChange([...value, row]);
  };
  ...
  return (
    <div className="space-y-4">
        ...
        {value.map((row, index) => (
          <div key={index} className="rounded-2xl border border-slate-200 bg-white p-4">
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {columns.map((column) => (
                <Input key={column.key} label={column.label} value={String(row[column.key] ?? '')} onChange={(e) => updateRow(index, column.key, e.target.value)} type={column.type === 'number' ? 'number' : column.type === 'date' ? 'date' : 'text'} />
              ))}
            </div>
      ...
      <Button variant="secondary" onClick={addRow}>{addButtonText || 'إضافة سطر'}</Button>
    </div>
  );
}
